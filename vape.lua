...
run(function()
	...
	local function getClosestTarget()
		...
		local screenCenter = vape.gui.AbsoluteSize / 2
		
		for _, ent in pairs(entitylib.GetEntities()) do
			if ent and ent[Part.Value] then
				local screenPos, onScreen = gameCamera:WorldToViewportPoint(ent[Part.Value].Position)
				if onScreen then
					local distance = (Vector2.new(screenPos.X, screenPos.Y) - screenCenter).Magnitude
					if distance < shortestDistance and distance <= FOV.Value then
		...
		Function = function(callback)
			...
			if callback then 
				AimAssist:Clean(runService.RenderStepped:Connect(function(dt)
					if CircleObject then 
						CircleObject.Position = vape.gui.AbsoluteSize / 2 -- Center the FOV circle
					end
					...
					if not vape.gui.ScaledGui.ClickGui.Visible then
						...
						if target then 
							...
							local new = (target[Part.Value].Position - gameCamera.CFrame.Position).Unit
	...
	AimAssist:CreateToggle({
		...
		Function = function(callback)
			if callback then
				...
				CircleObject.Position = vape.gui.AbsoluteSize / 2 -- Fixed at center
...
SilentAim = vape.Categories.Combat:CreateModule({
    ...
    end,  -- Closing the function for the 'SilentAim' module
...
CircleTransparency = SilentAim:CreateSlider({
    ...
    end  -- Closing the CreateSlider function block
})  -- Closing the CreateSlider call
run(function()
	local TriggerBot
	local Targets
	local ShootDelay
